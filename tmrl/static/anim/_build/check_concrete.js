/* visual check: screenshot frame_concrete.html at given frame indices (file://, frames inlined) */
...
(async () => {
  ...
  const idxs = (process.argv[4] || '9,120,220,330,430').split(',').map(s => parseInt(s, 10));
  ...
  for (const i of idxs) {
    ...
    const f = path.join(OUT, `concrete_${W}x${H}_f${i}.png`);
